...
-- Stock items
insert into rkj_stock_items (item_code,name,category,storage_unit,conversion_text,min_threshold,critical_threshold,status,notes) values ('ST-PLANTA','Roti Planta','Roti','Bag/Pcs','1 Bag = 20 pcs',NULL,NULL,'Active','Produk buatan kilang Roti Planta; digunakan oleh menu POS Roti Kaya') on conflict (item_code) do update set name=excluded.name, category=excluded.category, notes=excluded.notes, conversion_text=excluded.conversion_text;
