...
async function loadProfileTarget(
 ...
 id: string) {
 const { data, error } = await supabase.from('profiles').select('id, full_name, email, phone, role, status, legal_entity_id, branch_id, region_id').eq('id', id).eq('organization_id', organizationId).maybeSingle();
...
export async function PATCH(
 ...
 const profileLegalEntityId = (profile as typeof profile & { legal_entity_id?: string | null }).legal_entity_id ?? null;
 const groupWideAccess = canViewAllLegalEntities(profile.role);
 if (!groupWideAccess && existing.legal_entity_id !== profileLegalEntityId) {
 return NextResponse.json(
 { error: 'Akses ditolak untuk syarikat lain' },
 { status: 403 });
 }
 ...
 if (!groupWideAccess) {
 return NextResponse.json(
 { error: 'Hanya Pentadbir Utama/Admin boleh pindah syarikat' },
 { status: 403 });
 }
 ...
 return NextResponse.json(
 { error: 'Hanya Pentadbir Utama boleh tukar tahap akses staf' },
 { status: 403 });
 }
 ...
 return NextResponse.json(
 { error: 'Tidak boleh tukar role Pentadbir Utama dari HR' },
 { status: 400 });
 }
 ...
 const { data: updated, error } = await (service as SupabaseClient).from('profiles').update(updates).eq('id', id).eq('organization_id', profile.organization_id).select(
 'id, full_name, email, phone, role, status, legal_entity_id, legal_entity:legal_entities(code, name, legal_name)').single();

 if (error) return NextResponse.json({ error: error.message }, { status: 400 });

 const { data: linkedStaffRow } = await supabase.from('staff').select('id').eq('profile_id', id).eq('organization_id', profile.organization_id).maybeSingle();
 ...
 await (service as SupabaseClient).from('staff').update(staffUpdates).eq('id', linkedStaffId);
 }
 }
...
export async function DELETE(
 ...
 const profileLegalEntityId = (profile as typeof profile & { legal_entity_id?: string | null }).legal_entity_id ?? null;
 if (!canViewAllLegalEntities(profile.role) && existing.legal_entity_id !== profileLegalEntityId) {
 return NextResponse.json(
 { error: 'Akses ditolak untuk syarikat lain' },
 { status: 403 });
 }
 ...
 const { error } = await (service as SupabaseClient).from('profiles').update({ status: 'INACTIVE' }).eq('id', id).eq('organization_id', profile.organization_id);
 ...
 if (linkedStaffId) {
 await (service as SupabaseClient).from('staff').update({ status: 'INACTIVE' }).eq('id', linkedStaffId);
 }
