...
export async function POST(request: Request) {
  ...
  const catalog = await loadStockCatalog(service, profile.organization_id);
  ...
  const { data: order, error: orderErr } = await (service as SupabaseClient)
  ...
  return NextResponse.json({ order: { ...order, items: lineRows } });
}
