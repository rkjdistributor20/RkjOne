...
export async function POST(request: Request) {
  ...
  if (purpose === 'STOCK_ORDER') {
    const { data: order } = await (service as SupabaseClient)
      .from('agent_stock_orders')
      .select('total_amount_rm, status, agent_account_id')
      .eq('id', referenceId)
      .maybeSingle();
    ...
    const { data: sub } = await (service as SupabaseClient)
      .from('agent_outlet_subscriptions')
      .select('amount_rm, status, outlet:agent_outlets(agent_account_id)')
      .eq('id', referenceId)
      .maybeSingle();
    const subRow = sub as {
      amount_rm: number;
  ...
  const { data: payment, error } = await (service as SupabaseClient)
    ...
    .select('id, amount_rm, status, payment_method, purpose')
    .single();
  ...
  if (checkout.gateway_session_id) {
    await (service as SupabaseClient)
      .from('agent_online_payments')
      .update({ gateway_ref: checkout.gateway_session_id })
      .eq('id', payment.id);
  }
