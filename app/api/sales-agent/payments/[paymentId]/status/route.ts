...
export async function GET(
 ...
 const { data: payment } = await (service as SupabaseClient).from('agent_online_payments').select('id, agent_account_id, purpose, amount_rm, payment_method, status, paid_at, created_at').eq('id', paymentId).maybeSingle();
