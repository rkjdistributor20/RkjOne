...
export async function PATCH(
 ...
 const { data, error } = await (supabase as SupabaseClient).from('branches').update(updates).eq('id', id).eq('organization_id', profile.organization_id).select('id, branch_code, branch_name, status, area, region_id').single();

 if (error) return NextResponse.json({ error: error.message }, { status: 400 });

 if (body.status !== undefined && data) {
 await (supabase as SupabaseClient).from('inventory_locations').update({ is_active: body.status === 'ACTIVE', updated_at: new Date().toISOString() }).eq('branch_id', id).eq('location_type', 'BRANCH_KIOSK');
 }
