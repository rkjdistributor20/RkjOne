...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('stock_items')
    .select('id, item_code, name, min_threshold, critical_threshold, status, category, base_unit')
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await (supabase as SupabaseClient)
      ...
      .select('id, item_code, name, min_threshold, critical_threshold, status, category')
      .single();
