...
export async function PATCH(
 ...
 const { error } = await (supabase as SupabaseClient).from('stock_items').update(updates).eq('id', id).eq('organization_id', profile.organization_id);
...
export async function DELETE(
 ...
 const { error } = await (supabase as SupabaseClient).from('stock_items').delete().eq('id', id).eq('organization_id', profile.organization_id);
