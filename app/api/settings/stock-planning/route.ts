...
export async function GET() {
  try {
    ...
    const { data: settingsRow, error } = await supabase
      .from('org_stock_planning_settings')
      .select('stock_coverage_days, safety_buffer_pcs, updated_at')
    ...
    const { data: holidaysRow, error: holErr } = await supabase
      .from('malaysia_holidays')
      .select('holiday_date, name, holiday_type, demand_multiplier')
      .gte('holiday_date', today)
      .lte('holiday_date', in90)
      .order('holiday_date')
      .limit(12);
...
export async function PATCH(request: Request) {
  try {
    ...
    const { data: existing } = await supabase
      .from('org_stock_planning_settings')
      .select('stock_coverage_days, safety_buffer_pcs')
      .eq('organization_id', profile.organization_id)
      .maybeSingle();
    ...
    const { data: savedRow, error } = await (supabase as SupabaseClient)
      .from('org_stock_planning_settings')
