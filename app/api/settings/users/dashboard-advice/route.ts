...
export async function POST(request: Request) {
 ...
 let query = service.from('profiles').select('id, full_name, email, role, metadata, legal_entity_id, legal_entity:legal_entities(code)').eq('organization_id', profile.organization_id).eq('status', 'ACTIVE');
 ...
 ? await service.from('staff').select('profile_id, worker_type, status, legal_entity:legal_entities(code)').eq('organization_id', profile.organization_id).eq('status', 'ACTIVE').in('profile_id', profileIds)
 : { data: [] };
 ...
 const { error: updErr } = await (service as SupabaseClient).from('profiles').update({
