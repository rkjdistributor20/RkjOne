...
export async function GET() {
 ...
 const { data, error } = await (service as SupabaseClient).from('staff_payslips').select(
 'id, period_label, period_start, period_end, file_name, storage_path, created_at, source, gross_pay, net_pay, legal_entity:legal_entities(code, legal_name)').eq('profile_id', profile.id).order('created_at', { ascending: false });

 if (error) return NextResponse.json({ error: error.message }, { status: 500 });

 const rows = await Promise.all(
 (data ?? []).map(async (row) => {
 const { data: signed } = await service.storage.from(BUCKET).createSignedUrl(row.storage_path as string, 3600);
...
export async function POST(request: Request) {
 ...
 if (staffId) {
 const { data: staffRow } = await service.from('staff').select('id, profile_id').eq('id', staffId).eq('profile_id', profile.id).maybeSingle();
 if (!staffRow) {
 ...
 const { data: inserted, error: insertErr } = await (service as SupabaseClient).from('staff_payslips').insert({
