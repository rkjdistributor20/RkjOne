...
export async function GET(request: Request) {
 ...
 const base = supabase.from('delivery_orders').select('*', { count: 'exact', head: true }).eq('organization_id', profile.organization_id).gte('created_at', `${from}T00:00:00`).lte('created_at', `${to}T23:59:59`);
