...
export async function GET() {
 ...
 let query = (supabase as SupabaseClient).from('maintenance_reports').select(`
 ...
 const { data: branches } = await (supabase as SupabaseClient).from('branches').select('id').eq('organization_id', profile.organization_id).eq('region_id', profile.region_id);
...
export async function POST(request: Request) {
 ...
 if (profile.role === 'AREA_MANAGER' && profile.region_id) {
 const { data: branch } = await (supabase as SupabaseClient).from('branches').select('id').eq('id', branchId).eq('organization_id', profile.organization_id).eq('region_id', profile.region_id).maybeSingle();
 if (!branch) {
 return NextResponse.json({ error: 'Cawangan di luar kawasan anda' }, { status: 403 });
 }
 }

 const { data: numberData, error: numberError } = await (supabase as SupabaseClient).rpc('next_maintenance_report_number', { p_org_id: profile.organization_id });
 if (numberError) return NextResponse.json({ error: numberError.message }, { status: 500 });

 const { data: hanif } = await (supabase as SupabaseClient).from('profiles').select('id').eq('organization_id', profile.organization_id).eq('role', 'MAINTENANCE_MANAGER').ilike('full_name', '%HANIF%').maybeSingle();

 const substituteRequired = Boolean(body.substitute_required);

 const { data, error } = await (supabase as SupabaseClient).from('maintenance_reports').insert({
