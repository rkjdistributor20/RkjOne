...
export async function PATCH(
 ...
 const { data, error } = await (supabase as SupabaseClient).from('maintenance_reports').update(patch).eq('id', id).eq('organization_id', profile.organization_id).select('id, status, substitute_status').single();
