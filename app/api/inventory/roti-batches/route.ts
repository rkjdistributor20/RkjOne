...
export async function GET(request: Request) {
 ...
 `).eq('location_id', locationId).in('status', ['ACTIVE', 'EXPIRED']).gt('quantity_remaining', 0).order('production_date', { ascending: true });
 ...
 const batches: RotiBatchAtLocation[] = ((data ?? []) as unknown as Row[]).filter((row) => row.stock_item?.category === 'Roti').map((row) => {
