...
function rowStatus(
 qty: number,
 min: number | null,
 critical: number | null): StockStatus {
...
export async function GET(request: Request) {
 ...
 let branchQuery = supabase.from('branches').select('id, branch_code, branch_name, status').eq('organization_id', profile.organization_id).order('branch_code');
 ...
 const { data: locations, error: locErr } = await supabase.from('inventory_locations').select('id, name, branch_id, is_active').eq('organization_id', profile.organization_id).eq('location_type', 'BRANCH_KIOSK').in('branch_id', branchIds);
 ...
 const { data: pendingTransfers, error: trfErr } = await supabase.from('stock_transfers').select('id, to_location_id').eq('organization_id', profile.organization_id).eq('status', 'IN_TRANSIT').in('to_location_id', locationIds);
