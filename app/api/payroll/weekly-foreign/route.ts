...
export async function POST(request: Request) {
 ...
 if (runId) {
 await (service as SupabaseClient).from('payroll_runs').update({ report_type: 'WEEKLY_FOREIGN' }).eq('id', runId);
 }

 const { data: rules } = await supabase.from('payroll_rules').select('id, rule_code, worker_type, component, rate, period, shift_hours, status, notes').eq('organization_id', profile.organization_id).eq('status', 'ACTIVE');
 ...
 company.staff.filter((s) => s.worker_type === 'FOREIGN').map((s) => ({
