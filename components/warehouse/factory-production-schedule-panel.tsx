...
export function FactoryProductionSchedulePanel() {
  ...
  const weekDays = weekDayDates(weekStart);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { week } = await fetchProductionWeek(weekStart);
      if (week) {
  ...
  async function handleSave(publish: boolean) {
    if (publish && selectedDays.size === 0) {
    ...
    try {
      await saveProductionWeek({
        ...
        production_dates: [...selectedDays].sort(),
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button type="button" variant="ghost" size="sm" onClick={() => setWeekStart(mondayOfWeek())}>
          Minggu ini
        </Button>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {active ? 'Hari production ✓' : 'Tiada production'}
                  </p>
                </button>
              );
            })}
