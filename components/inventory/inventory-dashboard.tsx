...
export function InventoryDashboard() {
 ...
 useEffect(() => {
 fetchStockItems({ hq: true }).then(({ items }) => setStockItems(items)).catch(() => toast.error('Gagal memuatkan senarai stok'));
 }, []);
 ...
 <ModuleHeader
 title="Inventori"
 description={`Kilang ke ${HQ_DISTRIBUTOR_LABEL} ke ${LOGISTIK_LABEL} ke Kiosk - 9 item stok rasmi selaras di seluruh rantaian`}
 icon={Package}
 badges={
 ...
 <>
 {locationsLoading ? (
 <ModuleLoading rows={1} />) : locations.length === 0 ? (
 <EmptyState
 icon={Package}
