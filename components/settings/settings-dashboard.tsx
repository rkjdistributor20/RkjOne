...
export function SettingsDashboard() {
  ...
  const defaultTab = canViewStaff && !isAdmin ? 'staff' : 'products';
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Master data · stock thresholds · users
        </p>
      </div>
        ...
        <Tabs defaultValue={defaultTab}>
          <TabsList className="flex-wrap h-auto">
            ...
            <TabsTrigger value="stock" className="gap-1">
              <SlidersHorizontal className="h-4 w-4" /> Stock Thresholds
            </TabsTrigger>
            {canViewStaff && (
          ...
          <TabsContent value="stock" className="mt-4 space-y-3">
            {!isAdmin && role !== 'CEO_FACTORY' ? (
              <p className="text-sm text-muted-foreground">Admin access required to edit thresholds</p>
            ) : (
              stockItems.map((item) => (
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u.id} className="border-b">
                        ...
                        <td className="p-3">{u.branch?.branch_name ?? 'HQ'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
