...
import { Users, Package, Building2, SlidersHorizontal, Settings2, CalendarDays, Landmark, ShieldCheck } from 'lucide-react';
...
export function SettingsDashboard({ initialUsers }: Props = {}) {
 ...
 const canLoadCatalog = isAdmin || role === 'CEO_FACTORY' || role === 'OPERATION_MANAGER';
 const needsBranches =
 activeTab === 'branches' || activeTab === 'staff' || activeTab === 'users';

 if (canLoadCatalog && activeTab === 'products') {
 tasks.push(fetchSettingsProducts().then((pr) => setProducts(pr.products)));
 }

 if (canLoadCatalog && activeTab === 'stock') {
 tasks.push(fetchSettingsStockItems().then((st) => setStockItems(st.items)));
 }
 ...
 setLoading(false);
 }, [profile, isAdmin, canManagePersonnel, canViewStaff, role, activeTab, loadUsers]);
