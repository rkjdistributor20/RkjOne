...
function LaneCard({ lane, ui }: { lane: OperatingLane; ui: UiText }) {
    ...
    <article className={cn("rounded-xl border p-4 shadow-sm", tone.card)}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <Badge
              variant="outline"
              className={cn("text-[10px] font-semibold", tone.badge)}
            >
              {lane.code}
            </Badge>
      ...
      <div className="mt-3 grid gap-2">
        ...
        <div className="rounded-lg border bg-background/80 p-2.5">
          ...
          <p className="text-xs leading-relaxed text-foreground">
            {lane.sourceOfTruth.map(ui).join(", ")}
          </p>
        </div>
      </div>
...
export function RkjOperatingMap() {
  ...
  return (
    <SectionCard
      title={ui("AI Operating Map RKJ One")}
      description={ui(
      ...
      action={
        <Button size="sm" variant="outline" className="gap-1.5">
          ...
          {ui("Bank-grade Control")}
        </Button>
      }
    >
      <div className="grid gap-3 xl:grid-cols-4">
        {RKJ_OPERATING_LANES.map((lane) => (
          <LaneCard key={lane.code} lane={lane} ui={ui} />
        ))}
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-xl border bg-muted/20 p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold">{ui("Handoff Operasi")}</p>
              <p className="text-xs text-muted-foreground">
                {ui("Setiap pindahan mesti ada bukti dan owner tindakan.")}
              </p>
            </div>
          </div>
          <div className="grid gap-2">
            {RKJ_OPERATING_HANDOFFS.map((handoff) => (
              <div
                key={`${handoff.from}-${handoff.to}-${handoff.title}`}
              ...
              >
                <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
                  ...
                  <span className="text-foreground">{ui(handoff.title)}</span>
                </div>
                <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                  {ui("Bukti")}: {ui(handoff.proof)}
                </p>
                <p className="mt-1 text-xs leading-relaxed text-foreground/80">
                  {ui("Tindakan")}: {ui(handoff.ownerAction)}
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl border bg-background p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold">{ui("Control Guardrail")}</p>
              <p className="text-xs text-muted-foreground">
                {ui("Rule owner supaya sistem kekal kemas dan boleh audit.")}
              </p>
            </div>
          </div>
          <div className="space-y-2">
            {RKJ_BANK_GRADE_GUARDRAILS.map((rule, index) => (
              <div
                key={rule}
                className="flex gap-2 rounded-lg border bg-muted/15 p-2.5 text-xs leading-relaxed"
              >
