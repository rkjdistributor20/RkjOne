...
| M7.10 | Blocked | DevOps/Database | Production migration gate. | Staging is synchronized through `20260805170000`, but the 2026-08-05 Production audit found 11 remote-only migration versions absent from the repository and the CLI refused `db push --dry-run`. Do not repair history or push migrations until the remote-only changes are recovered and reviewed, the histories reconcile exactly, and backup, rollback authority and a maintenance window are confirmed. |
