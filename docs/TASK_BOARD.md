...
| M7.15 | Review | Mobile/POS/QA | Android 58 mm Bluetooth receipt printing. | The consolidated Android 1.6.9 candidate supports one-time pairing, saved/verified printer selection, direct/test/automatic printing, duplicate protection and delayed cash-drawer pulse for the photographed POS-5890U-L. Earlier device tests passed, but the exact consolidated Play-delivered bundle still requires a fresh pilot acceptance test before Production rollout. |
| M7.16 | Review | Security/Backend/Database | Close critical active-user, tenant-branch and POS immutability boundaries. | A migration now makes RLS helpers fail closed for inactive profiles, binds every branch decision to the caller organization and removes direct authenticated DML on completed POS financial rows. Personnel branch assignment also validates organization scope. Local clean replay and tenant assertions pass; staging is synchronized. Role/branch regression UAT and current recovery evidence remain required before Production. |
