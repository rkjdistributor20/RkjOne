...
| M7.16 | Validation | Security/Backend/Database | Close critical active-user, tenant-branch and POS immutability boundaries. | A migration now makes RLS helpers fail closed for inactive profiles, binds every branch decision to the caller organization and removes direct authenticated DML on completed POS financial rows. Personnel branch assignment also validates organization scope. Local clean migration replay and tenant-boundary assertions pass; staging deployment and role regression remain required before Production. |
