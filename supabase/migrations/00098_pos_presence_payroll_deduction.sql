-- POS presence payroll deduction
-- Migration 00098

ALTER TABLE payroll_line_items
 ADD COLUMN IF NOT EXISTS kiosk_excess_minutes NUMERIC(8, 2) NOT NULL DEFAULT 0,
 ADD COLUMN IF NOT EXISTS kiosk_deduction NUMERIC(10, 2) NOT NULL DEFAULT 0;

COMMENT ON COLUMN payroll_line_items.kiosk_excess_minutes IS
 'Total POS kiosk away excess minutes above daily allowance for this payroll period.';
COMMENT ON COLUMN payroll_line_items.kiosk_deduction IS
 'Payroll deduction generated from POS kiosk excess minutes.';
...
DECLARE
 ...
 v_kiosk_excess_minutes NUMERIC := 0;
 v_kiosk_deduction NUMERIC := 0;
 v_deduction_hourly_rate NUMERIC := 0;
BEGIN
 v_user_id := auth.uid();
 ...
 v_socso := 0;
 v_eis := 0;
 v_kiosk_excess_minutes := 0;
 v_kiosk_deduction := 0;
 v_deduction_hourly_rate := 0;
 ...
 SELECT COALESCE(SUM(excess_minutes), 0) INTO v_kiosk_excess_minutes
 FROM pos_staff_presence_logs
 WHERE staff_id = v_staff.id
 AND payroll_deductible = true
 AND status = 'RETURNED'
 AND left_at::date BETWEEN p_period_start AND p_period_end;
 ...
 v_deduction_hourly_rate := COALESCE(v_ot_rate, 0);
 v_kiosk_deduction := ROUND((v_kiosk_excess_minutes / 60.0) * v_deduction_hourly_rate, 2);
 v_gross := v_shift_pay + v_ot_pay;
 v_net := GREATEST(v_gross - v_kiosk_deduction, 0);
 ELSE
 IF v_days > 0 THEN
 ...
 v_deduction_hourly_rate := COALESCE(NULLIF(v_staff.monthly_amount, 0), v_basic_rate, 0) / 30.0 / 8.0;
 v_kiosk_deduction := ROUND((v_kiosk_excess_minutes / 60.0) * v_deduction_hourly_rate, 2);
 v_net := GREATEST(v_gross - v_epf - v_socso - v_eis - v_kiosk_deduction, 0);
 END IF;

 IF v_gross > 0 OR v_days > 0 OR v_kiosk_excess_minutes > 0 THEN
 INSERT INTO payroll_line_items (
 payroll_run_id, staff_id, worker_type,
 basic_salary, attendance_allowance, shift_pay, ot_pay, commission,
 epf, socso, eis, kiosk_excess_minutes, kiosk_deduction, gross_pay, net_pay,
 sales_total, hours_worked, ot_hours, notes
 ) VALUES (
 v_run_id, v_staff.id, v_worker,
 v_basic, v_allowance, v_shift_pay, v_ot_pay, v_commission,
 v_epf, v_socso, v_eis, v_kiosk_excess_minutes, v_kiosk_deduction, v_gross, v_net,
 NULLIF(v_sales, 0), NULLIF(v_hours, 0), NULLIF(v_ot_hours, 0),
 CASE
 WHEN v_kiosk_excess_minutes > 0 THEN
 'Potongan kiosk: ' || ROUND((v_kiosk_excess_minutes / 60.0)::numeric, 2) || ' jam melebihi elaun rehat POS'
 ELSE NULL
 END
 );

 v_total_gross := v_total_gross + v_gross;
 v_total_ded := v_total_ded + v_epf + v_socso + v_eis + v_kiosk_deduction;
...
GRANT EXECUTE ON FUNCTION generate_payroll_run(DATE, DATE, UUID) TO authenticated;
