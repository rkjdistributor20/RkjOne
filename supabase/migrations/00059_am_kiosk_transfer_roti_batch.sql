-- AM boleh rekod tarikh batch roti sedia ada semasa pindahan kiosk→kiosk (kawasan)

CREATE OR REPLACE FUNCTION can_set_roti_production_date_on_kiosk_transfer()
RETURNS BOOLEAN
LANGUAGE sql
STABLE
AS $$
...
GRANT EXECUTE ON FUNCTION can_set_roti_production_date_on_kiosk_transfer TO authenticated;
...
BEGIN
  ...
  LOOP
    ...
    IF is_roti_stock_item(v_stock.id) THEN
      ...
      END IF;
      IF v_kiosk_to_kiosk THEN
        IF NOT can_set_roti_production_date_on_kiosk_transfer() THEN
          RAISE EXCEPTION 'Tiada kebenaran tetapkan batch roti untuk pindahan kiosk';
        END IF;
      ELSE
