...
export function itemTotals(
 allocations: Array<{ items: RebalanceItemQty[] }>): Map<string, number> {
...
export function validateRebalancePlan(
 ...
 options?: { mode?: 'kiosk-kiosk' | 'hq-kiosk' }): { ok: true } | { ok: false; message: string } {
...
/** Pecahkan pelan ambil/hantar kepada pindahan kiosk ke kiosk individu */
export function buildTransferLegs(
 pickups: PickupAllocation[],
 drops: DropAllocation[]): TransferLeg[] {
 const legsMap = new Map<string, TransferLeg>();

 for (const itemCode of HQ_STOCK_ITEM_CODES) {
 const roti = isRotiItemCode(itemCode);

 const pickRemaining = pickups.flatMap((p) =>
 p.items.filter((i) => i.item_code === itemCode && i.quantity > 0).map((i) => ({
 ...
 const dropRemaining = drops.map((d) => ({
 ...
 item_code: itemCode,
 quantity: move,
 unit: pick.meta.unit,...(roti && pick.meta.production_date
 ? {
