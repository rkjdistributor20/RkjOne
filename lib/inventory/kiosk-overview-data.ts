...
export async function fetchKioskOverviewForBranches(
 ...
 orgId: string,
 branchIds: string[]): Promise<{ branches: KioskBranchOverviewRow[]; summary: KioskOverviewSummary }> {
 if (!branchIds.length) {
 return {
 branches: [],
 summary: {
 ...
 const { data: branchRows } = await supabase.from('branches').select('id, branch_code, branch_name').eq('organization_id', orgId).in('id', branchIds).order('branch_code');

 const branches = (branchRows ?? []) as BranchRow[];

 const { data: locations } = await supabase.from('inventory_locations').select('id, branch_id, is_active').eq('organization_id', orgId).eq('location_type', 'BRANCH_KIOSK').in('branch_id', branchIds);
 ...
 const { data: pendingTransfers } = await supabase.from('stock_transfers').select('to_location_id').eq('organization_id', orgId).eq('status', 'IN_TRANSIT').in('to_location_id', locationIds);
