  ...
  if (input.is_group_owner || employments.length >= 2) {
    ...
    if (input.is_group_owner || companies.length >= 2) {
      return advice(
        'OWNER_GROUP',
        '/dashboard',
        ['reports', 'payroll', 'hr', 'finance', 'fleet', 'approval'],
        'Pemilik / pengurusan merentas 3 syarikat legal — dashboard kumpulan dengan aliran kerja penuh.',
  ...
  switch (role) {
    case 'SUPER_ADMIN':
      return advice(
        ...
        '/dashboard',
        ['reports', 'payroll', 'hr', 'finance', 'fleet', 'approval'],
        'Pentadbir utama — paparan pemilik kumpulan 3 syarikat.',
    ...
    case 'OPERATION_MANAGER':
      return advice(
        ...
        'Pengurus operasi — prestasi kiosk, syif dan inventori.',
        ['RKJ']
      );
    case 'CEO_FACTORY':
      return advice(
        ...
        'CEO Kilang — pengeluaran, stok kilang dan pesanan.',
        ['RKJ_MFG']
      );
    case 'AREA_MANAGER':
      return advice(
    ...
    default:
      ...
      if (entity === 'RKJ_DIST') {
        return advice(
          ...
          '/inventory',
          ['stock_hq', 'fleet'],
          `Staf ${entityLabel('RKJ_DIST')} — pengedaran, stok HQ dan logistik.`,
          ['RKJ_DIST']
        );
      }
      return advice(
