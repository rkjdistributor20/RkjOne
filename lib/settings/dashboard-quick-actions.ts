...
const MODULE_HREF: Partial<Record<PermissionModule, { href: string; label: string; description: string }>> = {
 ...
 hr: { href: '/hr', label: 'HR & Gaji Syarikat', description: 'Maklumat staf & payroll' },
...
export function staffQuickActionsFromMetadata(metadata: unknown) {
 ...
 return modules.map((m: PermissionModule) => MODULE_HREF[m]).filter((item): item is NonNullable<typeof item> => Boolean(item));
}
