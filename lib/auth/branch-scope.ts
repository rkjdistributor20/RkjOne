...
export async function resolveScopedBranches(
 ...
 const { data: regionBranches } = await supabase.from('branches').select('id').eq('organization_id', profile.organization_id).eq('region_id', profile.region_id);
