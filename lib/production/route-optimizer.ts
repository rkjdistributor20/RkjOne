...
export function groupPickByCategory(
 items: DriverWorkScheduleEntry['pick_summary']): Array<{ category: string; items: typeof items }> {
 ...
 return ['Roti', 'Bahan', 'Packaging', 'Lain'].filter((c) => map.has(c)).map((c) => ({ category: c, items: map.get(c)! }));
}
