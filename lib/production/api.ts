...
export async function fetchFactoryRawMaterials() {
 return fetchJson<FactoryRawMaterialDashboard>('/api/production/raw-materials');
}

export async function recordFactoryRawMaterialUsage(payload: {
 production_date: string;
 items: Array<{ stock_item_id: string; received_qty?: number; used_qty?: number }>;
 notes?: string;
}) {
