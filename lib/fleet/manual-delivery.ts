...
export function aggregateInstructionItems(
 instructions: ManualDeliveryInstructionInput[]): Array<{ stock_item_id: string; quantity: number }> {
...
export function buildManualDeliveryLegs(options: {
 ...
 to_location_id: fleetSlotId,...legMeta,
 items: allItems,
 });
 } else {
 legs.push({
 ...
 to_location_id: row.destId,...legMeta,
...
export function createEmptyManualInstructions(
 ...
 defaultItemId: string): ManualDeliveryInstruction[] {
...
export function validateManualInstructions(
 instructions: ManualDeliveryInstructionInput[]): { ok: true } | { ok: false; message: string } {
