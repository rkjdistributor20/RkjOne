...
function buildNarrativeSummary(
 ...
 periodLabel: string,
 companies: CompanyPayrollProposal[],
 totals: AiPayrollProposal['totals']): string {
...
async function loadShiftAggregates(
 ...
 staffIds: string[],
 periodStart: string,
 periodEnd: string): Promise<Map<string, ShiftAgg>> {
 const map = new Map<string, ShiftAgg>();
 if (staffIds.length === 0) return map;

 const { data } = await supabase.from('staff_shifts').select('staff_id, ot_hours, actual_hours, scheduled_hours').in('staff_id', staffIds).eq('status', 'APPROVED').gte('shift_date', periodStart).lte('shift_date', periodEnd);
...
async function loadSalesByStaff(
 ...
 staffIds: string[],
 periodStart: string,
 periodEnd: string): Promise<Map<string, number>> {
 const map = new Map<string, number>();
 if (staffIds.length === 0) return map;

 const { data } = await supabase.from('pos_shifts').select('staff_id, total_sales').in('staff_id', staffIds).eq('status', 'CLOSED').gte('closed_at', `${periodStart}T00:00:00`).lte('closed_at', `${periodEnd}T23:59:59`);
...
function buildLocalLine(
 ...
 sales: number,
 periodStart: string,
 periodEnd: string): PayrollProposalLine {
 const flags: string[] = [];
...
function buildForeignLine(
 ...
 periodType: ProposalPeriodType): PayrollProposalLine {
 const flags: string[] = [];
...
export async function generateAiPayrollProposal(
 ...
 periodEnd: string,
 periodLabel: string,
 periodType: ProposalPeriodType): Promise<AiPayrollProposal> {
 ...
 const { data: staffMeta } = await supabase.from('staff').select('id, profile_id').in('id', allStaffIds.length ? allStaffIds : ['00000000-0000-0000-0000-000000000000']);

 const profileByStaff = new Map(
 (staffMeta ?? []).map((s) => [s.id as string, s.profile_id as string | null]));

 const { data: tierRows } = await supabase.from('commission_tiers').select('id, tier_from, tier_to, commission_amount, formula_description, status').eq('organization_id', organizationId).eq('status', 'ACTIVE');
...
export function companyForLine(
 ...
 staffId: string): CompanyPayrollProposal | undefined {
...
export function updateProposalLine(
 ...
 value: number): AiPayrollProposal {
 ...
 return recalculateCompanyTotals({...company,
