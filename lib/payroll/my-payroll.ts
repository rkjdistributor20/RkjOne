...
function buildPayBreakdown(
 ...
 monthlyAmount: number | null): Array<{ label: string; amount: number }> {
 ...
 return [
 { label: `Shift ${shiftHours}j × ${pay.shiftsPerWeek} hari`, amount: pay.weekly },...(otRule?.rate ? [{ label: 'OT (anggaran/jam)', amount: otRule.rate }] : []),
 ];
 }
 if (workerType === 'LOCAL') {
...
export async function getMyPayrollDashboard(
 ...
 rules: PayrollRule[]): Promise<MyPayrollDashboard> {
 const { data: profile } = await supabase.from('profiles').select('full_name, email, metadata, legal_entity_id').eq('id', profileId).single();

 const { data: staffRows } = await supabase.from('staff').select(
 'id, staff_code, full_name, worker_type, weekly_amount, monthly_amount, shift_hours, shifts_per_week, legal_entity_id, branch:branches(branch_name), legal_entity:legal_entities(code, legal_name)').eq('organization_id', organizationId).eq('profile_id', profileId).eq('status', 'ACTIVE');
 ...
 const { data: lines } = await supabase.from('payroll_line_items').select(
 'gross_pay, net_pay, shift_pay, ot_pay, basic_salary, commission, epf, socso, eis, staff_id, payroll_run:payroll_runs(run_number, period_start, period_end, status)').in('staff_id', staffIds).order('created_at', { ascending: false }).limit(24);
 ...
 const { data: payslipRows } = await supabase.from('staff_payslips').select(
 'id, period_label, period_start, period_end, file_name, storage_path, created_at, source, gross_pay, net_pay, legal_entity:legal_entities(code, legal_name)').eq('profile_id', profileId).order('created_at', { ascending: false }).limit(20);
