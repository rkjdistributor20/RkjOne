...
function calcCommission(tiers: CommissionTier[], sales: number): number {
 const match = tiers.filter((t) => t.status === 'ACTIVE' && sales >= t.tier_from && (t.tier_to == null || sales <= t.tier_to)).sort((a, b) => b.tier_from ?? a.tier_from)[0];
...
export function computeRkjSalesLocalPay(
 ...
 sales: number,
 periodStart: string,
 periodEnd: string): LocalPayComputation {
