...
export async function distributePayslipsFromProposal(
 ...
 publishedBy: string,
 payrollRunId?: string | null): Promise<DistributeResult> {
 ...
 const { data: legalEntity } = await service.from('legal_entities').select('id').eq('organization_id', organizationId).eq('code', company.company_code).maybeSingle();
