...
export async function getCompanyPayrollDashboard(
  ...
  rules: PayrollRule[]
): Promise<CompanyPayrollDashboard> {
  ...
  const { data: staffRows, error } = await supabase
    .from('staff')
    .select(
      'id, staff_code, full_name, worker_type, weekly_amount, monthly_amount, shift_hours, shifts_per_week, legal_entity_id, branch:branches(branch_code, branch_name), legal_entity:legal_entities(id, code, legal_name, scope)'
    )
    .eq('organization_id', organizationId)
    .eq('status', 'ACTIVE')
    .order('staff_code');
