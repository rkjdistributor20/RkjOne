...
export const POS_MENU_CATEGORIES: PosMenuCategory[] = [
...
export const POS_MENU_STOCK_CODES: Record<PosRotiMenuCategory, HqStockItemCode> =
  Object.fromEntries(
    RKJ_STOCK_CATALOG.filter((i) => i.pos_menu).map((i) => [i.pos_menu!, i.item_code])
  ) as Record<PosRotiMenuCategory, HqStockItemCode>;
...
/** Pecah jumlah pcs → bag penuh + baki pcs (ikut pack_quantity setiap jenis roti) */
...
/** Papar stok roti/plastik: contoh 42 pcs @ 20/bag → "2 bag · 2 pcs" */
...
/** Tukar kuantiti order (bag/tong) → unit asas (pcs/gram) */
export function toBaseQuantity(
  qty: number,
  item: Pick<RkjStockItemDef, 'pack_quantity' | 'base_unit'>
): number {
...
/** Tukar input reject (bag/tong) → kuantiti asas untuk API */
...
export function formatRejectPreview(
  ...
  return `→ ${quantity.toLocaleString('ms-MY')} ${label} akan ditolak`;
}
